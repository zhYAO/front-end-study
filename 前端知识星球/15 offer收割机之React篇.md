...
在使用表单来收集用户输入时，例如input、select、textearea等元素都要绑定一个change事件，当表单的状态发生变化，就会触发onChange事件，更新组件的state。这种组件在React中被称为**受控组件**，在受控组件中，组件渲染出的状态与它的value或checked属性相对应，react通过这种方式消除了组件的局部状态，使整个状态可控。react官方推荐使用受控表单组件。
...
调用 `setState` 时，组件的 `state` 并不会立即改变，react会将多个setState调用放在一个批处理（batch）中， `React` 会优化真正的执行时机，并出于性能原因，它会将更新作为一个任务放入任务队列中，这个任务会在合适的时候由React的调度器（Scheduler）处理。 最终更新只产生一次组件及其子组件的重新渲染，这对于大型应用程序中的性能提升至关重要。
在渲染过程中，React可能会构建一个并发树（concurrent tree），这棵树反映了应用在某一特定时间点的状态。这个过程是可中断的，React可以根据可用时间片来决定是否继续渲染或者暂停渲染以响应用户输入或其他高优先级任务
